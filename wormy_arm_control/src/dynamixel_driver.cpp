...
bool DynamixelDriver::open_port(void)
{
...
void DynamixelDriver::close_port(void)
{
  dxl_port_handler_->closePort();
}

std::string DynamixelDriver::get_last_error_log(void)
{
  return last_error_log_;
}
#include <iostream>
bool DynamixelDriver::torque_enable(const bool enable)
{
  bool retval = true;
...
bool DynamixelDriver::write_goal_joint_positions(const std::vector<double> & goal_positions)
{
...
bool DynamixelDriver::write_moving_speed_rpm(const uint8_t dxl_id, const double speed_rpm)
{
...
bool DynamixelDriver::write_moving_speed_rpm_all(const double speed_rpm)
{
  bool retval = true;
...
bool DynamixelDriver::read_present_joint_positions(std::vector<double> & joint_positions)
{
...
bool DynamixelDriver::read_present_joint_speeds(std::vector<double> & joint_speeds)
{
  std::vector<uint16_t> buffer;
...
bool DynamixelDriver::read_present_joint_loads(std::vector<double> & joint_loads)
{
  std::vector<uint16_t> buffer;
...
bool DynamixelDriver::read_present_joint_voltages(std::vector<double> & joint_voltages)
{
...
bool DynamixelDriver::read_present_joint_temperatures(std::vector<double> & joint_temperatures)
{
  std::vector<uint8_t> buffer;
...
bool DynamixelDriver::read_byte_data_from_each_joints(
...
bool DynamixelDriver::read_word_data_from_each_joints(
  const uint16_t address,
  std::vector<uint16_t> & buffer)
{
  bool retval = true;
...
bool DynamixelDriver::parse_dxl_error(
...
double DynamixelDriver::dxl_pos_to_radian(const uint16_t position)
{
  return (position - DXL_HOME_POSITION) * TO_RADIANS;
}

uint16_t DynamixelDriver::radian_to_dxl_pos(const double position)
{
  return position * TO_DXL_POS + DXL_HOME_POSITION;
}

double DynamixelDriver::dxl_speed_to_rps(const uint16_t speed)
{
...
double DynamixelDriver::dxl_load_to_percent(const uint16_t load)
{
...
double DynamixelDriver::dxl_voltage_to_actual_voltage(const uint8_t voltage)
{
